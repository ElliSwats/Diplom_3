...
class MainPage(BasePage):
    ...
    def get_text_create_burger(self):
        self.download_wait_by_visible(main_page_loc.check_text_constr)
        return self.get_text_by_element(main_page_loc.check_text_constr)

    @allure.step('Получить текст "Детали ингредиента"')
    def get_text_details_inrg(self):
        self.download_wait_by_visible(main_page_loc.ingredient_details)
        return self.get_text_by_element(main_page_loc.ingredient_details)
    ...
    @allure.step('Получить значение счетчика Флюоресцентная булка')
    def count_ingredient_fluorescent_bun(self):
        self.download_wait_by_visible(main_page_loc.totalizer_fluorescent_bun)
        return self.get_text_by_element(main_page_loc.totalizer_fluorescent_bun)
    ...
    @allure.step('Окно с деталями ингридиента закрыто')
    def ingr_details_window_close(self):
        self.invisible_element(main_page_loc.ingredient_details)
        self.find_element(main_page_loc.ingredient_details)
        return True
